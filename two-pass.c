...
/**********************************************************************/
void setUpTwoPassLpObjective( LP lp, AlphaList neighbor ) {
  int i;
  ...
  /* We will either subtract the neighbor from the vector or the
     vector from the neighbor depending upon the reward type being
     used. */
  ...
  for ( i = 0; i < gNumStates; i++ ) {
    ...
    lp->obj[i] = adjust_factor 
      * ( neighbor->alpha[i] - neighbor->prev_source->alpha[i] );
...
/**********************************************************************/
int numTwoPassLpNonZeroes( AlphaList node, 
...
/**********************************************************************/
void setUpTwoPassLpConstraints( LP lp, AlphaList node, 
                                AlphaList *projection ) {
  int row, col, z, index;
  AlphaList proj_node;
  double adjust_factor;
  
  /* We will either subtract the chsoen vector from the rest of the
     vectors in the projection[z] set or vice-versa depending upon the
     reward type being used. */
  if( gValueType == REWARD_value_type )
    adjust_factor = 1.0;
  else
    adjust_factor = -1.0;
...
/**********************************************************************/
LP setUpTwoPassLp( AlphaList neighbor, AlphaList *projection,
                   double sparse_epsilon ) {
/*
...
/**********************************************************************/
int feasibleTwoPassLp( AlphaList neighbor, 
                       AlphaList *projection,
...
/**********************************************************************/
void searchRegionTwoPass( AlphaList cur_vector, 
                          AlphaList list,
                          AlphaList *projection,
                          PomdpSolveParams param ) {
/* 
...
/**********************************************************************/
AlphaList improveTwoPass( AlphaList *projection, 
                          PomdpSolveParams param ) {
/*
