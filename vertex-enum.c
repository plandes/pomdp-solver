...
/**********************************************************************/
int initVertexEnum( int num_orig_variables ) {
/*
...
*/
  ...
  lp->obj = (double *) malloc ( sizeof( double ) * lp->cols );
  lp->lowbnd = (double *) malloc ( sizeof( double ) * lp->cols );
  lp->upbnd = (double *) malloc ( sizeof( double ) * lp->cols );
  lp->matbeg = (int *) malloc ( sizeof( int ) * lp->cols );
  lp->matcnt = (int *) malloc ( sizeof( int ) * lp->cols );
...
/**********************************************************************/
int basisToBits( unsigned int *basis_bits, int *basis ) {
/*
...
/**********************************************************************/
int bitsToBasis( int *basis, unsigned int *basis_bits ) {
/*
...
/**********************************************************************/
int basisEqual( unsigned int *basis1, unsigned int *basis2 ) {
/*
...
/**********************************************************************/
Record_List createRecordNode( double y, unsigned int *basic_slacks ) {
/*
...
*/   
   ...
   /* allocate memory */
   node = (Record_List) malloc( sizeof( *node ) );
   node->basic_slacks 
      = (unsigned int *) malloc( gBasicSlackBytes * sizeof( unsigned int ));
...
/**********************************************************************/
Record_List addRecordUnique( Record_List list,
                            double y,
...
/**********************************************************************/
Record_List dequeueRecord( Record_List *list ) {
/*
...
/**********************************************************************/
Record_List getFirstUnusedRecord( Record_List list ) {
...
/**********************************************************************/
void destroyRecordNode( Record_List node ) {

   if( node == NULL )
      return;

   free( node->basic_slacks );
   free( node );

}  /* destroyRecordNode */
/**********************************************************************/
Record_List cleanRecordList( Record_List list, double y ) {
/*
...
/**********************************************************************/
void destroyRecordList( Record_List list ) {
/*
...
/**********************************************************************/
/**********************************************************************/
void allocateVertexEnumLP( LP lp, int num_orig_vars, 
                           int num_constraints ) {
/* Allocate variables needed by CPLEX that will change from one
   ...
   lp->rhs = (double *) malloc ( sizeof( double ) * lp->rows );
   lp->sense = (char *) malloc ( sizeof( char ) * lp->rows );
   lp->matind = (int *) malloc ( sizeof( int ) * lp->matspace );
   lp->matval = (double *) malloc ( sizeof( double ) * lp->matspace );

}  /* allocateVertexEnumLP */
/**********************************************************************/
void freeVertexEnumLP( LP lp ) {
...
/**********************************************************************/
void allocateTempMemory( LP lp ) {
   int i;

   /* allocate temporary variables to be used throughout enumeration */
   vertex = (double *)malloc( gNumVariables * sizeof( double ));
   row_basis = (int *) malloc( gNumConstraints * sizeof( int ));
   col_basis = (int *) malloc( (gNumVariables+1) * sizeof( int ));
   gCurSlackBasis = (int *) malloc( gNumConstraints * sizeof( int ));
   ...
   /* This array will remain constant except for the last element
      ...
      */
   gColBasis = (int *) malloc( (gNumVariables + 1) * sizeof( int ));
   for( i = 0; i < gNumVariables; i++ )
      gColBasis[i] = BASIC;

   bit_basis = (unsigned int *)calloc( gBasicSlackBytes,
                                      sizeof( unsigned int ));
   ...
   x_vals = (double *)malloc( (gNumVariables + 1) * sizeof( double ));
   slack_vals = (double *)malloc( gNumConstraints * sizeof( double ));
   tableaux_col = (double *)malloc( gNumConstraints * sizeof( double ));
   tableaux_row = (double *)malloc( gNumConstraints * sizeof( double ));
   tableaux_rhs = (double *)malloc( gNumConstraints * sizeof( double ));
   tableaux_bv = (int *) malloc( gNumConstraints * sizeof( int ));

   row_ratio_winners = (int *) malloc( gNumConstraints * sizeof( int ));

   dummy_double = (double *) malloc( (lp->cols + lp->rows) * sizeof( double ));
   dummy_int = (int *) malloc( (lp->cols + lp->rows) * sizeof( int ));
...
/**********************************************************************/
int cplexCallback( CPXLPptr lpinfo, int wherefrom ) {
/*
...
/**********************************************************************/
int setTableaux( LP lp, int *slack_basis, int y_basis ) {
/*
...
/**********************************************************************/
int getTableauxColumn( LP lp, int col, double *coefs ) {
/*
...
/**********************************************************************/
int getTableauxRHS( LP lp, double *coefs ) {
/*
...
/**********************************************************************/
int findPivotRow( LP lp, int col, int *rows_to_pivot ) {
/*
...
/**********************************************************************/
int findBasicVariable( LP lp, int row, int* type, int *index ) {
/*
...
/**********************************************************************/
int verifyBasis( LP lp, int *row_basis ) {
/*
...
/**********************************************************************/
int doUSPivot( LP lp, int entering_var, int leaving_var ) {
/* 
...
/**********************************************************************/
int doUYPivot( LP lp, int entering_var, double *vertex ) {
/*
...
/**********************************************************************/
int doPivotCheck( LP lp, int col, double *b ) {
/*
...
/**********************************************************************/
int specialVertexCheck( LP lp, double *vertex ) {
/*
...
*/
int setUpIndices( LP lp, int num_constraints ) {
/* 
...
/**********************************************************************/
int setUpSimplexConstraints( LP lp, int num_constraints ) {
   int col;
...
/**********************************************************************/
int setUpRelaxedRegion( LP lp, AlphaList item, AlphaList list ) {
/*
   ...
   while( list != NULL ) {
      ...
      if( item != list ) {
         ...
         for( col = 0; col < gNumStates; col++ ) {
            ...
            /* I assume gValueType is reward, if not we need to add a check and
               switch the subtraction (if costs).
               */
            ...
            else  /* the POMDP uses a cost criteria */
               lp->matval[ index ] 
                 = item->alpha[col] - list->alpha[col];
...
/**********************************************************************/
int startVertexEnum( AlphaList item, AlphaList list ) {
/*
...
/******************       Debugging Routines        *******************/
/**********************************************************************/
void displayVertexEnumLPSolution( LP lp ) {
  LP_writeSolution( lp, "temp-sol" );
}  /* displayVertexEnumLPSolution */
/**********************************************************************/
void displayVertexEnumLP( LP lp ) {
   LP_writeLP( lp, "temp" );
}  /* displayVertexEnumLP */
/**********************************************************************/
int showVertex( double *b ) {
   int i;
