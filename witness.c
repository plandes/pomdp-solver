...
void initWitness( ) {
  ...
  gTempWitness = (double *) malloc( gNumStates * sizeof( double ));
...
/**********************************************************************/
AlphaList improveWitness( AlphaList *projection,
...
*/
  ...
  while ( ( cur_agenda_item = extractUnmarkedVector( agenda )) 
    ...
    /* This is the main thing: see if agenda item leads to a witness
       point. This condition call the simplex empty region check
    ...
    if ( ! isEmptyRegionSimpleCheck( new_alpha_list, 
      ...
      /* In theory, we should never get a NULL node here, since we in
         ...
         dthe precision we use to compare vectors might not be
