...
/**********************************************************************/
EpochStats newEpochStatNode( int epoch_num ) {
/*
  Creates a new structure node and initializes the values.
*/
  EpochStats epoch_stats;
  int i;

  epoch_stats = (EpochStats) malloc( sizeof( *epoch_stats ));

  epoch_stats->epoch_num = epoch_num;

  for ( i = 0; i < MAX_NUM_CONTEXT_TYPES; i++ )
    epoch_stats->epoch_time[i] = INVALID_TIME;

  epoch_stats->user_time = INVALID_TIME;
  epoch_stats->system_time = INVALID_TIME;
...
/**********************************************************************/
EpochStats getEpochStats( SolutionStats stat,
                          int epoch_num ) {
  EpochStats node;
...
/**********************************************************************/
EpochStats getOrCreateEpochStats( SolutionStats stat,
                                  int epoch_num ) {
/*
...
/**********************************************************************/
void recordEpochSetEpsilon( int epoch_num,
                           double set_epsilon,
                           SolutionStats stat ) {
/*
...
/**********************************************************************/
void recordEpochSolutionSize( int epoch_num,
                              int solution_size,
                              SolutionStats stat ) {
/*
...
/**********************************************************************/
void recordEpochMaxEpsilon( int epoch_num,
                            double max_epsilon,
                            SolutionStats stat ) {
/*
...
/**********************************************************************/
void recordEpochTime( int epoch_num,
                      double user_time,
                      double system_time,
                      SolutionStats stat ) {
/*
...
/**********************************************************************/
void recordEpochContextTime( int epoch_num,
                             int context_num,
                             double time,
                             SolutionStats stat ) {
/* 
...
/**********************************************************************/
void incrementEpochContextTime( int epoch_num,
                             int context_num,
                             double time,
                             SolutionStats stat ) {
/*
...
*/
  ...
  /* First check to see if there is an invalid time.  This means it
     should have really started at zero. */
  if ( Equal( epoch_stat->epoch_time[context_num],
              INVALID_TIME, SMALLEST_PRECISION ))
    epoch_stat->epoch_time[context_num] = time;
  
  /* Otherwise we just accumulate the time. */
  else
    epoch_stat->epoch_time[context_num] += time;

}  /* incrementEpochContextTime */
/**********************************************************************/
void reportEpochSummary( SolutionStats stat ) {
...
/**********************************************************************/
SolutionStats newSolutionStats( FILE *report_file, 
                                int succinct,
                                int stat_summary ) {
/*
  ...
  statistics and whether or not to be succinct in the reporting.  */
  SolutionStats stat;
  int i;

  stat = (SolutionStats) malloc( sizeof( *stat ));

  stat->report_file = report_file;
  stat->succinct = succinct;
...
/**********************************************************************/
void destroySolutionStats( SolutionStats stat ) {
/*
...
/**********************************************************************/
void epochStartStats( SolutionStats stat ) {
/*
  Called just prior to doing the value iteration new value function
  computation.  Sets up the stats in preparatio for this iteration. 
*/
  int i;
...
/**********************************************************************/
void epochEndStats( SolutionStats stat, int solution_size ) {
/*
...
*/
  ...
  /* Add them to the global time. */
  stat->tot_time_user += epoch_time_user;
  stat->tot_time_system += epoch_time_system;
 
  if ( stat->succinct == TRUE )
    fprintf( stat->report_file, "V=%d T=%.3lf ", 
             solution_size,
             epoch_time_user + epoch_time_system );

  /* If not succinct reporting */
  else
    fprintf(stat->report_file, 
            "%d vectors. (%.2lf secs.) (%.2lf secs. total)\n", 
            solution_size,
            epoch_time_user + epoch_time_system, 
            stat->tot_time_user + stat->tot_time_system );
...
/**********************************************************************/
void recordLpStats( SolutionStats stat,
                    int num_variables, int num_constraints ) {
/*
...
/**********************************************************************/
void getLpStats( SolutionStats stat, 
                 int *tot_lps, int *tot_constraints ) {
/*
...
/**********************************************************************/
void reportLpStats( SolutionStats stat ) {
/*
...
*/
  ...
  for ( context = 0; context < MAX_NUM_CONTEXT_TYPES; context++ ) {
    ...
    if ( stat->succinct == TRUE )
      fprintf( stat->report_file,
               "%slp=%d %sc=%d ", 
               context_type_str_brief[context], 
...
/**********************************************************************/
void startContext( SolutionStats stat, ContextType context ) {
/*
...
/**********************************************************************/
void endContext( SolutionStats stat, ContextType context ) {
/*
...
/**********************************************************************/
void reportContextDetails( SolutionStats stat ) {
/* 
...
*/
  ...
  for ( context = 0; context < MAX_NUM_CONTEXT_TYPES; context++ ) {
    ...
    if ( stat->succinct == TRUE )
      fprintf( stat->report_file, "%st=%.2f ", 
               context_type_str_brief[context], time );
    ...
    if ( stat->succinct == TRUE )
      fprintf( stat->report_file, "Ct=%.2f ", 
               tot_time );
    else
...
/**********************************************************************/
void reportStats( SolutionStats stat ) {
/*
