...
/* An enumeration type for the various algorithms that can be used
   here. */
#define MAX_NUM_METHODS         5
typedef enum { 
  Enumeration, 
  TwoPass, 
  LinearSupport, 
...
/* There is more than one way to determine if we are done with
   value iteration.  The 'exact' method is very stringent; you
   must get the exact vectors in the exact order.  The weak just 
   look for what the maximum value difference between the two sets
   would be. */
#define MAX_NUM_STOP_CRITERIA          2
typedef enum { stop_exact, stop_weak } StopCriteria;
#define STOP_CRITERIA_STRINGS     { \
                                      "exact", \
                                      "weak" \
                                  }
...
#define MAX_VI_VARIATION_TYPES                    3
...
struct PomdpSolveParamStruct {
  ...
  /* We'll keep track of the epoch (iterayion) as value iteration
     progresses. */
  int cur_epoch;

  /* When we are running experiments and want to accumulate data, we
     would only like to have the bare bones of information.  When this
     is specified each epoch will output only the number of vectors
     and number of seconds for that epoch.  The exception is the
     witness algorithm which will precede the number of vectors and
     the time with the number of Q vectors for each action in the
     action number order.  This is used when we have turned the
     verboseness off and takes presedence over any other verbose
     flag. */
  int succinct;
  ...
  /* The criteria to use to know when to stop an infinite horizon
     problem. */
  ...
  char prefix_str[MAX_FILENAME_LENGTH];  /* Prefix for output filenames */
  ...
  /* These are use for auitomatic epsilon adjustment. */
...
#define DEFAULT_WEAK_BOUND_DELTA        1e-9
#define DEFAULT_METHOD                  IncrementalPruning
#define DEFAULT_STOP_CRITERIA           stop_weak
...
extern char *method_str[];
extern char *stop_criteria_str[];
...
extern PomdpSolveParams parseCmdLineParams( int argc, 
                                            char **argv, 
                                            int *mark_args  );

/* Shows all the current settings and parameters for this run of the
  program.  */
extern void showPomdpSolveParams( PomdpSolveParams param  );

/* Displays usage information in response to the '-h' or -'help'
  command line options.  */
extern void showUsageParams(  );
